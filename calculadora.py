...
st.write("Ingrese la capacidad de pago del cliente para realizar la búsqueda exacta de las ofertas disponibles.")

# --- BASE DE DATOS DE BÚSQUEDA DIRECTA (DATOS ESTÁTICOS DE LOS PDF) ---
# Nota: Esta estructura consulta los valores textuales exactos sin fórmulas matemáticas.
DATA_CREDITOS = [
    # Mas Nómina (MN 4766) - Escala oficial
    ...
    # Mas Nómina (MN 3772) - Escala oficial
    ...
    # Opcipres (OPC 4689) - Escala oficial
    ...
    # Consubanco (CSB 4707) - Escala oficial
...
if capacidad_num is not None and capacidad_num > 0:
    ...
    # Búsqueda/Filtrado directo sobre los registros exactos del PDF
    ...
    if df_viables.empty:
        ...
        # Búsqueda del mayor monto alcanzable por cada marca y plazo
        idx_mejores = df_viables.groupby(["Marca", "Plazo_Meses"])["Monto"].idxmax()
        resultados = df_viables.loc[idx_mejores].copy()

        # Ordenar por Tasa de mayor a menor y por plazos descendentes
        ...
        # Despliegue en tarjetas agrupadas por marca
